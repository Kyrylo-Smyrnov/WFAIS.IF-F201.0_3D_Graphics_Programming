...
void SimpleShapeApplication::init() {
    ...
    GLuint u_buffer_handle_0;
    OGL_CALL(glCreateBuffers(1, &u_buffer_handle_0));
    OGL_CALL(glNamedBufferData(u_buffer_handle_0, 8 * sizeof(float), NULL, GL_STATIC_DRAW));
    OGL_CALL(glNamedBufferSubData(u_buffer_handle_0, 0 * sizeof(float), 1 * sizeof(float), &strength));
    OGL_CALL(glNamedBufferSubData(u_buffer_handle_0, 4 * sizeof(float), 3 * sizeof(float), &mix_color));

    GLuint u_buffer_handle_1;
    OGL_CALL(glCreateBuffers(1, &u_buffer_handle_1));
    OGL_CALL(glNamedBufferData(u_buffer_handle_1, 12 * sizeof(float), NULL, GL_STATIC_DRAW));
    OGL_CALL(glNamedBufferSubData(u_buffer_handle_1, 0 * sizeof(float), 2 * sizeof(float), &scale));
    OGL_CALL(glNamedBufferSubData(u_buffer_handle_1, 2 * sizeof(float), 2 * sizeof(float), &trans));
    OGL_CALL(glNamedBufferSubData(u_buffer_handle_1, 4 * sizeof(float), 2 * sizeof(float), &rot[0]));
    OGL_CALL(glNamedBufferSubData(u_buffer_handle_1, 8 * sizeof(float), 2 * sizeof(float), &rot[1]));
    ...
    OGL_CALL(glBindBufferBase(GL_UNIFORM_BUFFER, 0, u_buffer_handle_0));
    OGL_CALL(glBindBufferBase(GL_UNIFORM_BUFFER, 1, u_buffer_handle_1));
